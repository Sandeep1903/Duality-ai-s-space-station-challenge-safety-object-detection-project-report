# Duality-ai-s-space-station-challenge-safety-object-detection-project-report
project for hackthaon
...
├── runs/                        # YOLOv8 training outputs and checkpoints
│   └── exp_space/               # Experiment run outputs (e.g., weights/best.pt)
├── reports/                     # Evaluation reports and artifacts
│   └── eval_exp_space/          # Test set evaluation results (predictions, confusion matrix, etc.)
├── visualizations/              # Auto-generated visualizations (loss curves, PR curves, etc.)
├── classes.txt                  # List of object categories (1 per line)
├── results.csv                  # Training log output from YOLOv8
├── train.py                     # YOLOv8 training script (reads yolo_params.yaml)
├── predict.py                   # Model evaluation and prediction script
├── visualize.py                 # Script to generate visual reports/graphs
├── yolo_params.yaml             # All hyperparameters and paths for YOLOv8 training
├── training_summary_TRAIN.json  # Training summary (metrics, settings)
├── training_summary_EVAL.json   # Evaluation summary (metrics, settings)
├── requirements.txt             # All pip dependencies
├── streamlit_direct_ngrok.py    # Streamlit app with ngrok support for inference
├── yolov8.ipynb                 # Training & inference notebook (Colab or local)
├── Space Station Object Detection System.pptx  # Final presentation slide deck
└── README.md                    # This file
```
...
- [x] Failure case analysis and model refinement
